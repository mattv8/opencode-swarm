...
> Historical note: v7.16.0 supersedes the Stage B dispatch details below. Standard non-Lean `parallelization` is now config-driven, and conditional adversarial `test_engineer` is included in the Stage B gate group when required.
- **EXECUTION DEFAULTS block**: New hardening section in architect prompt with 5 permanent rules: infinite time/resources, parallel coder authorization (up to 3), Stage B dispatch hardening, mandatory drift checks, anti-pressure stance.
